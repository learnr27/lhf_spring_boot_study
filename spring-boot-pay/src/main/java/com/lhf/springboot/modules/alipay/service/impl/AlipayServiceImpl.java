package com.lhf.springboot.modules.alipay.service.impl;
...
import com.lhf.springboot.common.utils.CommonUtil;
...
public class AlipayServiceImpl implements AlipayService {
    ...
    public String alipay(Product product) {
        ...
        String sellerId = "";
        //(可选商户门店编码，通过门店号和商家后台可以配置精准到门店的折扣信息，详询支付宝技术支持
        ...
        // 支付超时，定义为120分钟  该笔订单允许的最晚付款时间，逾期将关闭交易。
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl(notify_url);   //支付宝服务器主动通知客户服务器里指定的页面http路径，根据需要设置
    ...
    public String aliRefund(Product product) {
        ...
        String outTrandeNo = product.getOutTradeNo();
        //(必填)退款金额，该金额必须小于等于订单的支付金额，单位为元
        String refundAmound = CommonUtil.divide(product.getTotalFee(), "100").toString();
        ...
        AlipayTradeRefundRequestBuilder builder = new AlipayTradeRefundRequestBuilder()
                .setOutTradeNo(outTrandeNo)
                .setRefundAmount(refundAmound)
            ...
            if(file.exists()){
                ...
                if(response.isSuccess()){
                    ...
                    if("ACQ.TRADE_NOT_EXIST".equals(response.getSubCode())){
                        logger.info("订单号："+product.getOutTradeNo()+response.getSubMsg()+"(预下单 未扫码的情况)交易不存在");
                        message = Constants.FAIL;
    ...
    public String aliPayMobile(Product product) {
        ...
        bizContent.put("body", "两个苹果五毛钱");  //商品描述
    ...
    public String aliPayPc(Product product) {
        ...
        bizContent.put("total_amount", product.getTotalFee()); //订单金额：元
        bizContent.put("subject", product.getSubject());   //订单标题
        bizContent.put("seller_id", Configs.getPid());   //实际收款账户，一般填写商户PID即可
        bizContent.put("product_code", "FAST_INSTANT_TRADE_PAY");  //电脑网址支付
        bizContent.put("body", "商品描述信息");
        /**
         * 这里有三种模式可供选择
    ...
    @Override
    public String aliPayQuery(Product product) {
        String serverUrl = "https://openapi.alipay.com/gateway.do";

        AlipayClient alipayClient = new DefaultAlipayClient(serverUrl, Configs.getAppid(), Configs.getPrivateKey(), "json", AliPayConfig.CHARSET, Configs.getAlipayPublicKey(), "RSA2"); //获得初始化的AlipayClient
        AlipayTradeQueryRequest request = new AlipayTradeQueryRequest();   //创建API对应的request类
        JSONObject bizContent = new JSONObject();
        bizContent.put("out_trade_no", product.getOutTradeNo());   //商户订单号
